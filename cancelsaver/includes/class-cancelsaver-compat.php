<?php
/**
 * CancelSaver Compatibility Layer
 *
 * Detects which subscription plugin is active and provides a
 * unified API so the rest of CancelSaver doesn't care which plugin
 * the store is using.
 *
 * Supported plugins:
...
class CancelSaver_Compat {
     ...
     * Returns a CancelSaver_Subscription wrapper or null.
     */
        ...
        switch ( self::$active_plugin ) {
            case 'wcs':
                if ( function_exists( 'wcs_get_subscription' ) ) {
                    ...
                    if ( $sub ) return new CancelSaver_Subscription_WCS( $sub );
                }
                break;
            ...
            case 'webtoffee':
                ...
                if ( $order && $order->get_type() === 'wt_subscription' ) {
                    return new CancelSaver_Subscription_WebToffee( $order );
                }
                // Fallback: try loading directly
                if ( $order ) return new CancelSaver_Subscription_WebToffee( $order );
                break;
            ...
            case 'yith':
                ...
                if ( $post && $post->post_type === 'yith_subscription' ) {
                    return new CancelSaver_Subscription_YITH( $id );
                }
                break;

            case 'sumo':
                return new CancelSaver_Subscription_SUMO( $id );

            case 'pluginshive':
                return new CancelSaver_Subscription_Generic( $id );
        }
        return null;
    }
            ...
            case 'webtoffee':
                ...
                add_action( 'woocommerce_order_status_changed', function( $order_id, $from, $to ) use ( $callback ) {
                    if ( in_array( $to, [ 'cancelled', 'wc-cancelled' ] ) ) {
                        $sub = CancelSaver_Compat::get_subscription( $order_id );
                        if ( $sub ) call_user_func( $callback, $sub );
                    }
                }, 10, 3 );
                break;
 ...
 */
abstract class CancelSaver_Subscription_Base {
...
class CancelSaver_Subscription_WCS extends CancelSaver_Subscription_Base {
    private $sub;
    ...
    public function pause( $months ) {
        ...
        $this->sub->update_meta_data( '_cancelsaver_resume_date', $resume );
        $this->sub->save();
        wp_schedule_single_event( strtotime( "+{$months} months" ), 'cancelsaver_resume_sub', [ $this->get_id() ] );
        return true;
    }
...
class CancelSaver_Subscription_WebToffee extends CancelSaver_Subscription_Base {
    private $order;
    ...
    public function pause( $months ) {
        ...
        update_post_meta( $this->get_id(), '_cancelsaver_resume_date', date( 'Y-m-d H:i:s', strtotime( "+{$months} months" ) ) );
        wp_schedule_single_event( strtotime( "+{$months} months" ), 'cancelsaver_resume_sub', [ $this->get_id() ] );
        return true;
    }
    public function skip_payment() {
    ...
    public function apply_discount( $amount, $type ) {
        ...
        update_post_meta( $this->get_id(), '_cancelsaver_coupon', $code );
        return $code;
    }
    public function cancel() {
...
class CancelSaver_Subscription_YITH extends CancelSaver_Subscription_Base {
    private $id;
    ...
    public function pause( $months ) {
        ...
        update_post_meta( $this->id, '_cancelsaver_resume_date', date( 'Y-m-d H:i:s', strtotime( "+{$months} months" ) ) );
        wp_schedule_single_event( strtotime( "+{$months} months" ), 'cancelsaver_resume_sub', [ $this->id ] );
        return true;
    }
    public function skip_payment() {
        $next     = $this->get_next_payment_date();
    ...
    public function apply_discount( $amount, $type ) {
        ...
        update_post_meta( $coupon_id, 'customer_email', [ $this->get_customer_email() ] );
        update_post_meta( $coupon_id, 'expiry_date', date( 'Y-m-d', strtotime( '+30 days' ) ) );
        update_post_meta( $this->id, '_cancelsaver_coupon', $code );
        return $code;
    }
    public function cancel() {
...
class CancelSaver_Subscription_SUMO extends CancelSaver_Subscription_Base {
    private $id;
    public function __construct( $id ) { $this->id = $id; }
    public function get_id()             { return $this->id; }
...
class CancelSaver_Subscription_Generic extends CancelSaver_Subscription_Base {
    private $id;
    public function __construct( $id ) { $this->id = $id; }
    public function get_id()             { return $this->id; }
    public function get_customer_id()    { return (int) get_post_meta( $this->id, '_customer_user', true ); }
...
add_action( 'cancelsaver_resume_sub', 'cancelsaver_do_resume_sub' );
function cancelsaver_do_resume_sub( $sub_id ) {
    $sub = CancelSaver_Compat::get_subscription( $sub_id );
    if ( ! $sub ) return;

    switch ( CancelSaver_Compat::$active_plugin ) {
        case 'wcs':
            $raw = wcs_get_subscription( $sub_id );
            if ( $raw && $raw->has_status( 'on-hold' ) ) {
                $raw->update_status( 'active' );
                $raw->delete_meta_data( '_cancelsaver_resume_date' );
                $raw->save();
            }
            break;
        case 'webtoffee':
            ...
            if ( $order ) {
                ...
                delete_post_meta( $sub_id, '_cancelsaver_resume_date' );
            }
            break;
        case 'yith':
