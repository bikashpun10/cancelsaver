=== CancelSaver - Subscription Retention ===
Contributors:      cancelsaver
...
**CancelSaver intercepts WooCommerce subscription cancellations and shows a beautiful popup with retention offers before the subscriber leaves.**

Most stores lose subscribers silently — the customer clicks cancel, the subscription ends, and no one ever finds out why. CancelSaver stops that from happening.
...
* **Skip** — Skip their next payment to give them breathing room
* **Discount** — Offer a percentage or fixed discount to stay

If they still cancel, CancelSaver automatically sends a **win-back email** with a unique coupon code — giving you a second chance to recover that subscriber.

Everything is tracked in a clean dashboard so you can see exactly how much revenue you're saving.

= Why CancelSaver? =

* **Offer first, not a survey** — Competitors show a survey before the offer. CancelSaver shows the offer immediately, which gets higher acceptance.
...
* **Built-in win-back email** — Other plugins require Klaviyo or Mailchimp. CancelSaver sends beautiful win-back emails automatically.
...
CancelSaver does not collect, share, or transmit any personal data to external servers. All data is stored in your own WordPress database. The win-back email is sent using your own WordPress mail (wp_mail).

== Installation ==

1. Upload the plugin files to `/wp-content/plugins/cancelsaver/`, or install through the WordPress Plugins screen directly.
2. Activate the plugin through the **Plugins** screen in WordPress.
3. Make sure you have a supported subscription plugin active (WooCommerce Subscriptions, WebToffee, YITH, or SUMO).
4. The plugin is active immediately — go to **CancelSaver → Settings** to customize your offers.
5. Go to **CancelSaver → Dashboard** to track performance.

== Frequently Asked Questions ==

= Which subscription plugins are supported? =

CancelSaver works with WooCommerce Subscriptions (official), Subscriptions for WooCommerce by WebToffee, YITH WooCommerce Subscriptions, and SUMO Subscriptions. The correct plugin is detected automatically.
...
Yes. Go to CancelSaver → Settings to change the headline, subheadline, discount amount, and win-back email subject.

= Is this GDPR compliant? =

Yes. CancelSaver does not send any data to external servers. Win-back emails are sent using your WordPress installation's own email system. No third-party services are involved.

== Screenshots ==

1. The retention popup shown when a subscriber clicks cancel
2. The CancelSaver dashboard showing saves, revenue, and save rate
